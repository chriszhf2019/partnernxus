-- ═════════════════════════════════════════════════════════════════════════
-- PartnerNexus 真实业务种子数据 v2
-- 基于2025年真实市场数据填充
-- 数据来源：政府采购招标公告、行业研究报告、厂商激励政策
-- ═════════════════════════════════════════════════════════════════════════
...
('东软集团股份有限公司', 'Platinum', 'Cooperating', 'ISV', '陈明', '沈阳市浑南区东软软件园', '华北', '辽宁', '沈阳市', '2017-06-01', 8, 'Gold', ARRAY['ISV','医疗行业','信创入围'], 68, '医疗', true, '医疗行业核心ISV，具备全院级解决方案交付能力', '91210100701796354X'),
...
('福建顶点软件股份有限公司', 'Silver', 'Cooperating', 'ISV', '何军', '福州鼓楼区软件园', '华东', '福建', '福州市', '2022-02-28', 3, 'Registered', ARRAY['ISV','金融','证券','交易系统'], 45, '金融', false, '金融证券ISV，交易系统合作伙伴', '91350000154863458K'),
-- Registered 伙伴
...
('阿克苏地区第二人民医院医疗云服务', '阿疆阿克苏地区第二人民医院', 8500000,
...
- 方案入围：额外奖励2%
- 成功签约：额外奖励3%（单笔封顶20万）
适用产品：智慧医院整体解决方案、医疗云平台、AI辅助诊疗系统', '2025-07-01', '2025-09-30'),

-- AI新品首发激励
('AI新品首发合作伙伴激励', 'New Product', 'Active', 'Cash', 2000000, 680000, 25,
 '2025年AI新品首发专项激励：
- 首单AI产品奖励：渠道经理5万元/单
- AI项目规模奖励：合同金额>500万额外奖励3%
- 联合方案奖励：ISV+SI联合申报额外奖励2%
适用产品：AI视觉平台、智能客服、数据智能中台', '2025-06-15', '2025-09-15'),

-- 信创替代专项
('信创替代竞争性激励计划', 'Competitive', 'Active', 'Rebate', 5000000, 2100000, 45,
 '国产化替代专项激励（替换Oracle/IBM/SAP等）：
- 基础返点：12%
- 超过1000万项目：15%
- ISV联合方案：额外3%
- 成功交付奖励：单笔最高30万
覆盖行业：金融、政务、医疗、能源', '2025-07-01', '2025-12-31'),
...
- 新签Gold伙伴：奖励10万
- 新签Silver伙伴：奖励5万
- 存量伙伴业绩增长奖励：增长>30%额外奖励5%
已完成执行，实际发放485万', '2025-04-01', '2025-06-30'),

-- 下半年冲刺计划
('H2业绩冲刺激励计划', 'Sales Acceleration', 'Upcoming', 'Rebate', 4000000, 0, 0,
 '2025下半年业绩冲刺激励：
- Q3/Q4累计目标达成率>100%：额外奖励2%
- 年度黑马奖：增长最快Top5伙伴各奖20万
- 战略项目突破奖：单笔>2000万项目额外奖励10万
报名截止：2025-07-31', '2025-10-01', '2025-12-31'),
...
- 获得专家级认证：每人奖励2000积分
- 年度最佳赋能伙伴：奖励10万元市场基金
积分可兑换：培训课程、技术认证、 conference门票', '2025-04-01', '2025-12-31'),
...
- 保险核心系统云化：返点12%
- 金融AI应用：额外奖励3%
- 金融科技创新实验室伙伴：专属MDF额度20万', '2025-06-01', '2025-11-30'),
...
- 智慧社区/园区项目：返点8%
- 信创政务云项目：返点15%
- 联合解决方案（3家以上ISV）：额外5%奖励', '2025-05-01', '2025-10-31');
...
-- ─── 激励申请记录 ─────────────────────────────────────
INSERT INTO incentive_applications (plan_id, partner_id, partner_name, partner_tier, metric, claimed_value, payout_amount, status, submitted_at, approved_at)
SELECT 
    ip.id,
    p.id,
    p.name,
    p.tier,
    CASE WHEN RANDOM() < 0.4 THEN '商机报备' WHEN RANDOM() < 0.7 THEN '销售达成' WHEN RANDOM() < 0.9 THEN '客户转化' ELSE '培训参与' END,
    FLOOR(RANDOM() * ip.total_budget * 0.15)::NUMERIC + 50000,
    FLOOR(RANDOM() * ip.total_budget * 0.1)::NUMERIC + 30000,
    CASE 
        WHEN RANDOM() < 0.35 THEN 'approved'
        WHEN RANDOM() < 0.55 THEN 'paid'
        WHEN RANDOM() < 0.75 THEN 'pending'
        WHEN RANDOM() < 0.90 THEN 'reviewing'
        ELSE 'rejected'
    END,
    ip.start_date + (RANDOM() * (ip.end_date::date - ip.start_date::date))::INT,
    CASE WHEN RANDOM() < 0.6 THEN ip.start_date + (RANDOM() * 30)::INT ELSE NULL END
FROM incentive_programs ip
CROSS JOIN LATERAL (
    SELECT id, name, tier FROM partners 
    WHERE status = 'Cooperating' AND (tier IN ('Platinum', 'Gold', 'Diamond') OR RANDOM() < 0.3)
    ORDER BY RANDOM()
    LIMIT (5 + FLOOR(RANDOM() * 15))::INT
) p
WHERE ip.status IN ('Active', 'Ended');

-- 更新激励计划的实际申领数据
UPDATE incentive_programs ip SET
    claimed_amount = (
        SELECT COALESCE(SUM(ia.claimed_value), 0)
        FROM incentive_applications ia
        WHERE ia.plan_id = ip.id AND ia.status IN ('approved', 'paid')
    ),
    participants_count = (
        SELECT COUNT(DISTINCT ia.partner_id)
        FROM incentive_applications ia
        WHERE ia.plan_id = ip.id
    )
WHERE ip.status IN ('Active', 'Ended');
...
BEGIN
    ...
    SELECT COUNT(*) INTO incentive_count FROM incentive_programs;
    SELECT COALESCE(SUM(amount), 0) INTO mdf_total FROM mdf_allocations;
    
    RAISE NOTICE '════════════════════════════════════════════════════════════';
    RAISE NOTICE 'PartnerNexus 数据填充完成';
    RAISE NOTICE '════════════════════════════════════════════════════════════';
    RAISE NOTICE '合作伙伴数量: %', partner_count;
    RAISE NOTICE '商机报备数量: %', deal_count;
    RAISE NOTICE '激励计划数量: %', incentive_count;
    RAISE NOTICE 'MDF总配额: ¥%,即元', mdf_total;
    RAISE NOTICE '════════════════════════════════════════════════════════════';
END $$;
