  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        >
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <h4 className="text-sm font-black text-slate-900 uppercase tracking-widest">{t('marketing.essence')}</h4>
          </div>
          <p className="text-xs text-slate-500 leading-relaxed font-medium">
            {t('marketing.essenceDesc')}
          </p>
          <div className="mt-4 pt-4 border-t border-slate-50 flex items-center gap-2 text-[10px] font-black text-blue-600">
        ...
        >
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <h4 className="text-sm font-black text-slate-900 uppercase tracking-widest">{t('marketing.core')}</h4>
          </div>
          <p className="text-xs text-slate-500 leading-relaxed font-medium">
            {t('marketing.coreDesc')}
          </p>
          <div className="mt-4 pt-4 border-t border-slate-50 flex items-center gap-2 text-[10px] font-black text-emerald-600">
        ...
        >
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <h4 className="text-sm font-black text-slate-900 uppercase tracking-widest">{t('marketing.methods')}</h4>
          </div>
          <p className="text-xs text-slate-500 leading-relaxed font-medium">
            {t('marketing.methodsDesc')}
          </p>
          <div className="mt-4 pt-4 border-t border-slate-50 flex items-center gap-2 text-[10px] font-black text-amber-600">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
          ...
          <div className="mt-4 w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
            <div className="bg-blue-600 h-full rounded-full" style={{ width: `${(stats.usedAmount / stats.annualQuota) * 100}%` }}></div>
          </div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
          ...
          <div className="mt-4 flex items-center gap-1">
            {[1, 2, 3, 4, 5].map(i => (
              <div key={i} className={cn("h-1.5 flex-1 rounded-full", i <= 4 ? "bg-primary" : "bg-slate-100")} />
            ))}
          </div>
      ...
      <div className="grid grid-cols-12 gap-8">
        ...
        <div className="col-span-12 lg:col-span-4 bg-white p-8 rounded-3xl border border-slate-100 shadow-sm flex flex-col">
          <h4 className="text-sm font-black text-slate-900 uppercase tracking-widest mb-8 flex items-center gap-2">
            <BarChart3 className="w-4 h-4 text-primary" /> {t('marketing.funnel')}
          </h4>
          ...
          <div className="flex-1 flex flex-col justify-center space-y-4">
            {[
              ...
              { label: t('marketing.conversion'), value: '120K', percentage: 40, color: 'bg-primary' },
              { label: t('marketing.roi'), value: '¥2.4M', percentage: 25, color: 'bg-emerald-600' },
            ].map((step, idx) => (
              <div key={idx} className="relative group">
                ...
                <div className="w-full bg-slate-50 h-8 rounded-xl overflow-hidden border border-slate-100 relative">
                  <motion.div 
                    initial={{ width: 0 }}
        ...
        <div className="col-span-12 lg:col-span-8 bg-white p-8 rounded-3xl border border-slate-100 shadow-sm">
          <div className="flex items-center justify-between mb-8">
            <h4 className="text-sm font-black text-slate-900 uppercase tracking-widest flex items-center gap-2">
              <Calendar className="w-4 h-4 text-primary" /> 本月活动执行详情 (9月)
            </h4>
            <div className="flex items-center gap-4">
              ...
              <button className="text-[10px] font-black text-primary hover:underline">查看历史活动</button>
            </div>
          </div>
          
          <div className="space-y-4">
            {activities.map((activity) => (
              <div key={activity.id} className="p-5 rounded-2xl border border-slate-50 hover:border-slate-200 hover:bg-slate-50/30 transition-all group">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm">
          <h4 className="text-sm font-black text-slate-900 uppercase tracking-widest mb-6 flex items-center gap-2">
            <Briefcase className="w-4 h-4 text-primary" /> {t('marketing.toolkit')}
          </h4>
          <div className="grid grid-cols-2 gap-4">
            {[
              ...
              <div key={idx} className="p-4 rounded-2xl border border-slate-50 hover:border-blue-100 hover:bg-blue-50/20 transition-all cursor-pointer group">
                <span className="text-xl mb-2 block">{tool.icon}</span>
                <p className="text-xs font-black text-slate-900 mb-1 group-hover:text-blue-600 transition-colors">{tool.label}</p>
        ...
        <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm">
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={idx} className="flex items-center justify-between p-4 rounded-2xl bg-slate-50/50 border border-transparent hover:border-slate-100 hover:bg-white transition-all cursor-pointer group">
                <div className="flex items-center gap-3">
                  <div className="w-1.5 h-1.5 rounded-full bg-amber-400" />
                  <div>
                    <p className="text-xs font-bold text-slate-900 group-hover:text-primary transition-colors">{post.title}</p>
