...
export default function PartnerDetailPage() {
  ...
  useEffect(() => {
    setLoading(true);
    partnerService.list().then(({ items }) => {
      setPartners(items);
      setLoading(false);
    }).catch(() => {
      setLoading(false);
  ...
  const refresh = useCallback(async () => {
    ...
    } catch { debug.warn('[PartnerDetailPage] refresh failed'); }
    setRefreshing(false);
  }, []);
  ...
  const handleImport = (imported: Partner[], mode: 'replace' | 'merge') => {
    if (mode === 'replace') {
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate('/partners')} className="p-2 rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors">
              <ArrowLeft className="w-5 h-5" />
            </button>
            <div>
              <h1 className="text-xl font-semibold text-neutral-900 dark:text-white">合作伙伴详情</h1>
              <p className="text-sm text-neutral-500 mt-1">管理合作伙伴生态 · 点击伙伴名称预览画像 · 使用自动分层快速筛选</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { icon: Clock, label: '待批复停留', value: `${pendingCount} 家`, sub: `最长 ${(() => { const days = partners.filter(p => p.status === 'Prospective').map(p => Math.ceil((Date.now() - new Date(p.applicationDate || p.startDate).getTime()) / 86400000)).filter(d => !isNaN(d) && isFinite(d)); return days.length > 0 ? Math.max(...days) : 0; })()} 天`, tip: '点击查看每家等待天数', color: 'text-amber-600 bg-amber-50', path: '/detail/partners-summary' },
