...
export const PipelineBoard: React.FC<PipelineBoardProps> = ({ onNavigate }) => {
    ...
    const fetchPipelineData = async () => {
      try {
        ...
        deals.forEach(deal => {
          ...
          const stage = deal.stage || deal.status || 'Registered';
          const isOpen = !isDealWon(deal) && !isDealLost(deal);
        ...
        const wonDeals = deals.filter(d => d.stage === 'ClosedWon' || d.status === 'Closed Won' || d.status === 'Converted');
        ...
        // Calculate win rate: won / (won + lost + open)
        const openDeals = deals.filter(d => d.stage !== 'ClosedWon' && d.stage !== 'ClosedLost' && d.status !== 'Closed Won' && d.status !== 'Closed Lost' && d.status !== 'Converted');
        const lostDeals = deals.filter(d => d.stage === 'ClosedLost' || d.status === 'Closed Lost');
        ...
        setPipelineData(getEmptyPipelineData(config.currency || 'CNY'));
      }
    };
  ...
  // Helper function for fallback - return empty data with indicator
  const getEmptyPipelineData = (currency: string) => {
    return {
      stages: config.salesStages.map((stage) => ({
        name: stage,
        amount: 0,
        inflow: 0,
        outflow: 0,
      ...
      winRate: 0,
      avgCycleDays: 0,
      sourceBreakdown: { selfReported: 0, assigned: 0 },
      isEmpty: true, // 标记为空数据
    };
  };
  ...
  const currency = config.currency || 'CNY';
  const isEmpty = (pipelineData as any)?.isEmpty || totalPipeline === 0;
      ...
      <div className="w-full lg:w-1/4 p-5 space-y-6">
        <div>
          ...
          <h3 className="text-4xl font-black text-blue-900 tracking-tight">
            {isEmpty ? (
              <span className="text-slate-400 text-lg">数据待更新</span>
