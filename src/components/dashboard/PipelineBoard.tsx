...
export const PipelineBoard: React.FC<PipelineBoardProps> = ({ onNavigate }) => {
  ...
  return (
    <motion.div 
      ...
      className="w-full bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden flex flex-col lg:flex-row divide-y lg:divide-y-0 lg:divide-x divide-slate-100"
    >
      {/* Left Section: 25% - 大盘与商机水源 */}
      <div className="w-full lg:w-1/4 p-5 space-y-6">
        <div>
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">本季度 Open Pipeline</p>
        ...
        <div className="pt-5 border-t border-slate-50">
          <div className="flex justify-between items-center mb-3">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">水源分布 (Source Breakdown)</span>
            <Activity className="w-3 h-3 text-slate-300" />
          </div>
          <div className="h-2.5 w-full flex rounded-full overflow-hidden mb-4 bg-slate-100">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <p className="text-xs font-black text-slate-900 ml-3">65%</p>
            </div>
            <div className="space-y-1">
              <div className="flex items-center gap-1.5">
              ...
              <p className="text-xs font-black text-slate-900 ml-3">35%</p>
            </div>
          </div>
        </div>

        <button onClick={() => onNavigate?.('deals')} className="w-full mt-auto py-2 bg-slate-50 text-slate-500 text-[10px] font-bold rounded-lg hover:bg-slate-100 transition-colors flex items-center justify-center gap-1">
      ...
      <div className="w-full lg:w-2/4 p-5 bg-slate-50/20 flex flex-col">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">销售阶段流转动态 (Sales Funnel Velocity)</h4>
          </div>
        </div>
          ...
          {funnelStages.map((stage, idx) => (
            <React.Fragment key={stage.name}>
              <div className={cn(
                ...
                stage.warning ? "border-red-100 shadow-sm shadow-red-50" : "border-slate-100 hover:border-blue-200 hover:shadow-md"
              )}>
                <p className="text-[9px] font-bold text-slate-400 mb-2 truncate" title={stage.name}>{idx + 1}. {stage.name}</p>
                <p className="text-sm font-black text-slate-900 mb-3 tracking-tight">{stage.amount}</p>
                <div className="flex flex-col gap-1">
                  ...
                  <div className="flex items-center justify-between px-1.5 py-0.5 bg-slate-50 text-slate-400 text-[8px] font-bold rounded border border-slate-100">
                ...
                <div className="absolute bottom-0 left-0 w-full h-1 bg-slate-50 rounded-b-xl overflow-hidden">
                  <div 
                    className={cn("h-full transition-all duration-1000", idx === 4 ? "bg-green-500" : "bg-blue-500")} 
      ...
      <div className="w-full lg:w-1/4 p-5 flex flex-col divide-y divide-slate-100">
        ...
        <div className="pb-5">
          <div className="flex items-center justify-between mb-4">
            <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">本季度营收达成率</h4>
        ...
        <div className="pt-5 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <p className="text-[9px] font-bold text-slate-400 uppercase">渠道整体赢单率</p>
            ...
            <div className="space-y-1">
              <p className="text-[9px] font-bold text-slate-400 uppercase">平均成交周期</p>
              <div className="flex items-baseline gap-1.5">
        ...
        <button onClick={() => onNavigate?.('marketing')} className="w-full mt-auto py-2 bg-slate-50 text-slate-500 text-[10px] font-bold rounded-lg hover:bg-slate-100 transition-colors flex items-center justify-center gap-1">
